...
const handleAddToCart = (productId: string, selectElement: HTMLSelectElement, name: string, short_description: string, mrp: number, sale_price: number, image: string) => {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-3 space-y-6 xl:col-span-3">
            <Button className="w-10 h-10 float-right rounded-full bg-blue-500 text-white relative flex items-center justify-center cursor-pointer" size="sm" onClick={openModal}>
    ...
    <Modal isOpen={isOpen} onClose={closeModal} className="max-w-6xl p-6">
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 text-gray-500 dark:text-gray-400">
            <svg className="w-16 h-16 mb-4 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.5 4.5A2 2 0 005.5 18H16a2 2 0 002-2v-.5a1 1 0 00-1-1H4" />
            </svg>
          ...
          <div className="flex flex-col h-full gap-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 flex-1 overflow-y-auto pr-2">
              {cartItems.map((item, index) => (
                <div key={item.product_id || index} className="group rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] hover:shadow-xl hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-300">
                  <div className="flex-shrink-0 mb-3">
                    <img 
                      className="w-20 h-20 object-cover rounded-xl border shadow-sm group-hover:scale-[1.02] group-hover:shadow-md transition-all duration-300 bg-gray-100 dark:bg-gray-800" 
                      src={item.image} 
                      alt={item.name}
                      onError={(e) => { 
                        e.currentTarget.style.display = 'none';
                  ...
                  <div className="space-y-2">
                    <h5 className="font-semibold text-sm text-gray-800 dark:text-white/95 truncate">
                      {item.name}
                    </h5>
